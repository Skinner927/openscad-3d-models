// Original box from: https://www.thingiverse.com/thing:3406724
// Parametric electronics box - OpenSCAD by JB86

$fn=100;

board = [55, 65, 6]; // (6 == thickness for usb)
speaker_h = 100; // Height of speaker cutout
fudge = [-2, 0, 0];
...
w=board.x + abs(fudge.x);
//inner box length + tolerances
h=board.y + chs_o1*2 + wt*2 + fudge.y;
//inner box depth
d=15;
...
translate([w/2+10,0,0]) {
  difference() {
    ...
    #translate([(board.x/-2) + fudge.x + wt/2, (board.y/-2) + fudge.y, bpt])
      board_cutout();


    // usb cord hole
    usb_d = 6;
    translate([(w/-2) + 13, h/2, d+(usb_d/2)])
    hull() {
      jj = usb_d;
      for(z=[jj, -jj])
        translate([0, 0, z])
          rotate([90, 0, 0])
            cylinder(d=usb_d, h=30, center=true);
    }
  }
}
...
module board_cutout() {
  ...
  // punch out for USB
  translate([wt*-2, 0, 0])
    cube([wt*2, 25, board.z]);

  // Speaker cutout
  translate([38, 48, speaker_h/-2])
    difference() {
      cylinder(d=30, h=speaker_h);

      // Slice grill lines
      translate([0, 0, speaker_h/2]) {
        for (i = [0:6:35])
          for (x = [i, -i])
            translate([x, 0, 0])
              cube([3, 35, speaker_h+5], center=true);
      }
    }
}
